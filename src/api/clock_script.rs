/// The JavaScript that installs a fake clock on `window.__browserlaneClock`.
...
pub const CLOCK_SCRIPT: &str = r#"() => {
	if (window.__browserlaneClock) return 'already_installed';
	...
	window.__browserlaneClock = clock;
	return 'installed';
}"#;
